...
def auth(login: str, password: str, db_conn_str: str, users_table: Table) -> User | None:
    engine = create_engine(db_conn_str, future=True)
...
def get_markets(db_conn_str: str, markets_table: Table) -> pd.DataFrame:
    ...
    markets_df["link1"] = (
        markets_df.loc[markets_df.index.intersection(markets_df["link1"])].reindex(markets_df["link1"])["name"].values
    )
    markets_df["link2"] = (
        markets_df.loc[markets_df.index.intersection(markets_df["link2"])].reindex(markets_df["link2"])["name"].values
    )
    return markets_df
...
def get_stocks(db_conn_str: str, stocks_table: Table) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        dict_results,
        columns=[stocks_table.c.price.name, stocks_table.c.ticket.name, stocks_table.c.cycle.name],
    )
...
def make_auto_green_investment(
    ...
    engine = create_engine(db_conn_str, future=True)
    last_green_markets = (
        prev_investments_df[prev_investments_df["income"] > 0].groupby("uid")["market"].apply(set).reset_index()
    )
    current_investments = (
        investments_df[investments_df["status"] == 1].groupby("uid")["market"].apply(set).reset_index()
    )
