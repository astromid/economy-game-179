...
from qd_cyberpank_game.db import (
    finish_cycle,
...
def finish_cycle_callback():
    ...
    make_next_cycle(
        ...
        st.session_state.fund_speed_change,
        st.session_state.db_schema["cycles"],
    )
...
def manual_transaction_callback():
    ...
    make_transaction(
        st.session_state.db_conn_str,
        st.session_state.db_schema["transactions"],
        transaction,
    )
...
def manual_multiplier_callback():
    ...
    bid_ids = investment_df[
        (investment_df["uid"] == uid) & (investment_df["market"] == market)
    ]["id"].tolist()
...
def root_dashboard():
    with st.sidebar:
        st.number_input(
            ...
            min_value=0.0,
            help="Fund speed",
            key="fund_speed_change",
            value=st.session_state.fund_speed,
        )
    ...
    st.markdown(
        f"### Скорость инвестирования капитала: ${st.session_state.fund_speed / 1000000:.2f} млн. / мин",
    )
    investments_df = st.session_state.investments
    users = st.session_state.users
    uid2name = {
        record["uid"]: record["name"]
        for record in users[["uid", "name"]].to_dict("records")
    }
    stocks_block()
    with st.beta_expander(label="Одобрение заявок в SEC", expanded=True):
        ...
        pending_bids = pending_bids.drop(
            ["uid", "timestamp_created", "timestamp_approved", "income", "status"],
            axis=1,
        )
        pending_bids = pending_bids[
            ["id", "corporation", "market", "amount", "multiplier", "cycle"]
        ]
    ...
    with st.beta_expander(label="Ручное управление"):
        ...
        with col1:
            with st.form("transactions_form"):
                ...
                st.selectbox(
                    "From",
                    options=users["uid"].tolist() + ["root"],
                    key="manual_transaction_from",
                )
                st.selectbox(
                    "To",
                    options=users["uid"].tolist() + ["root"],
                    key="manual_transaction_to",
                )
                st.number_input("Amount", step=1.0, key="manual_transaction_amount")
                st.form_submit_button(
                    "Провести транзакцию",
                    on_click=manual_transaction_callback,
                )
        with col2:
            with st.form("multiplier_form"):
                ...
                st.selectbox(
                    "Рынок",
                    options=st.session_state.markets["name"],
                    key="manual_multiplier_market",
                )
                st.number_input("Множитель", min_value=0.0, key="manual_multiplier")
                st.form_submit_button(
                    "Установить модификатор",
                    on_click=manual_multiplier_callback,
                )
