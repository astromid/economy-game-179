...
def make_markets_graph(
    ...
    graph_links = [
        ...
        for link in markets_df.melt(id_vars="name", value_vars=["link1", "link2"])
        .dropna()
        .to_dict("records")
    ]
    for node in graph_nodes:
        node_investment = approved_investments[
            approved_investments["market"] == node["name"]
        ]
        if not node_investment.empty:
            ...
            node[
                "income_str"
            ] = f"${(node['income'] / 1000000):.2f} млн. ({node['income'] / node['amount']:.2%})"
            node["multiplier"] = 1
        else:
            node["amount"] = 0
    ...
    for link in graph_links:
        ...
        if (
            source_color == ThemeColors.GREEN.value
            or target_color == ThemeColors.GREEN.value
        ):
...
def markets_status():
    ...
    graph = make_markets_graph(
        markets_df=markets_df,
        investments_df=prev_investments_df,
        home_market=st.session_state.user.home_market,
    )
    col1, col2 = st.beta_columns([1, 1])
    with col1:
        st.markdown(
            '<p style="text-align: center;"> Состояние графа рынков в прошлом периоде </p>',
            unsafe_allow_html=True,
        )
        st_pyecharts(graph, height="500px")
    with col2:
        st.markdown(
            '<p style="text-align: center;"> Графики инвестиций по рынкам за прошлый период </p>',
            unsafe_allow_html=True,
        )
        investments_nonzero_df = prev_investments_df[
            (prev_investments_df["amount"] != 0) & (prev_investments_df["status"] == 1)
        ]
        if investments_nonzero_df.empty:
            st.markdown(
                '<p style="text-align: center;"> В прошлом периоде инвестиций не было </p>',
                unsafe_allow_html=True,
            )
        else:
            investments_melt = investments_nonzero_df[
                ["market", "amount", "income"]
            ].copy()
            investments_melt["amount"] /= 1000000
            investments_melt["income"] /= 1000000
            investments_melt = investments_melt.melt("market")
            st.altair_chart(
                alt.Chart(investments_melt)
                .mark_bar()
                .encode(
                    ...
                    color=alt.condition(
                        alt.datum.value > 0,
                        alt.value(ThemeColors.GREEN.value),
                        alt.value(ThemeColors.RED.value),
                    ),
                )
                .properties(width=80, height=400),
            )
    st.markdown(
        f"### Инвестиции {st.session_state.user.name} Corporation за предыдущий период",
    )
...
def markets_investment_form():
    ...
    if not investments_df.empty:
        frozen_markets = investments_df.loc[
            investments_df["status"].isin([-1, 1]),
            "market",
        ].tolist()
    ...
    with st.form(key="investments_form"):
        ...
        for market in markets_df["name"].values:
            ...
            with col1:
                st.number_input(
                    label=f"Инвестиции в {market} (млн. $)",
                    key=f"investment_{market}",
                    min_value=0.0,
                    step=0.5,
                )
            with col2:
                ...
                if not investments_df.empty:
                    ...
                    bid = active_investments.loc[
                        active_investments["market"] == market,
                        "status",
                    ]
                    if not bid.empty:
                        bid_status = bid.item()
                ...
                st.markdown(
                    f"## <br /><p> Статус: {status_str} </p>",
                    unsafe_allow_html=True,
                )
        st.form_submit_button(
            label="Отправить заявку в SEC",
            on_click=markets_investment_callback,
        )
        if st.session_state.frozen_markets:
            st.warning(
                f"На следующие рынки уже поданы заявки: {', '.join(frozen_markets)}. Эти заявки не будут обновлены.",
            )
        rejected_bids = getattr(st.session_state, "rejected_bids", None)
        if rejected_bids:
            st.warning(
                f"Заявки на следующие рынки были отклонены из-за недостаточного баланса: {', '.join(rejected_bids)}",
            )
