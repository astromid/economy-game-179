...
def update_game_state(uid: str) -> None:
    ...
    with placeholder.beta_container():
        ...
        st.session_state.cycle, st.session_state.fund_speed = db.get_current_cycle(
            db_conn_str,
            db_schema["cycles"],
        )
        ...
        st.session_state.transactions = db.get_transactions(
            db_conn_str,
            uid,
            db_schema["transactions"],
        )
        progressbar.progress(40)
        st.session_state.prev_investments = db.get_cycle_investments(
            db_conn_str,
            uid,
            st.session_state.cycle - 1,
            db_schema["investments"],
        )
        progressbar.progress(50)
        st.session_state.investments = db.get_cycle_investments(
            db_conn_str,
            uid,
