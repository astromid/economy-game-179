...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("market", sa.Integer, sa.ForeignKey("markets.id"), nullable=False),
        sa.Column("amount", sa.Integer, nullable=False),
        sa.Column("delivered_amount", sa.Integer),
    )
    op.create_table(
        "player_modificators",
        sa.Column("cycle", sa.Integer, sa.ForeignKey("cycles.id")),
    ...
    balances_view_sql = """
        ...
        SELECT cycle, user, SUM(amount) AS balance
        FROM transactions
        GROUP BY cycle, user
    """
    warehouses_view_sql = """
        ...
        SELECT
            ...
            w.user,
            w.market,
            SUM(w.amount) OVER (PARTITION BY w.user, w.market ORDER BY w.cycle, w.user, w.market) AS amount
        FROM (
            SELECT ps.cycle, ps.user, ps.market, SUM(ps.amount) AS amount
            FROM (
                SELECT p.cycle, p.user, p.market, SUM(p.amount) AS amount
                FROM production_log p
                GROUP BY p.cycle, p.user, p.market
                UNION ALL
                SELECT s.cycle, s.user, s.market, -SUM(COALESCE(s.delivered_amount, s.amount)) AS amount
                FROM supplies s
