...
def calculate_investments(
    ...
    approved_invest_bids = investments_df[
        (investments_df["status"] == 1) & investments_df["income"].isnull()
    ].copy()
    if approved_invest_bids.empty:
        return None

    approved_invest_bids["duration"] = (
        datetime.now() - approved_invest_bids["timestamp_approved"]
    )
    approved_invest_bids["funded_amount"] = (
        fund_speed * approved_invest_bids["duration"].dt.seconds / 60
    )
    approved_invest_bids["funded_amount"] = approved_invest_bids[
        "multiplier"
    ] * approved_invest_bids[["funded_amount", "amount"]].min(axis=1)
    ...
    for market in markets:
        ...
        market_capacity = markets_df.loc[
            markets_df["name"] == market,
            "capacity",
        ].item()
        min_market_capacity = markets_df.loc[
            markets_df["name"] == market,
            "min_capacity",
        ].item()
        incomes, new_market_capacity = calculate_market(
            funded_amounts,
            market_capacity,
            min_market_capacity,
        )
...
def generate_stocks(
    market_incomes: list[dict[str, Any]] | None,
    ...
    if market_incomes is None:
        ...
        for market_income in market_incomes:
            ticket = users_df.loc[
                users_df["uid"] == market_income["uid"],
                "ticket",
            ].item()
