...
def read_db_schema(db_conn_str: str) -> dict[str, MetaData | Table]:
...
def auth(
    ...
    stmt = select(
        users_table.c.uid,
        users_table.c.name,
        users_table.c.ticket,
        users_table.c.home_market,
    )
...
def get_current_cycle(db_conn_str: str, cycles_table: Table) -> tuple[int, float]:
    engine = create_engine(db_conn_str, future=True)
    stmt = select(cycles_table.c.cycle, cycles_table.c.fund_speed).order_by(
        cycles_table.c.timestamp.desc(),
    )
    with engine.connect() as conn:
        result = conn.execute(stmt).first()
    return (
        (result.cycle, float(result.fund_speed))
        if result is not None
        else (1, DEFAULT_FUND_SPEED)
    )
...
def make_next_cycle(
    ...
    engine = create_engine(db_conn_str, future=True)
    stmt = insert(cycles_table).values(
        cycle=current_cycle + 1,
        timestamp=datetime.now(),
        fund_speed=fund_speed,
    )
    with engine.connect() as conn:
        conn.execute(stmt)
        conn.commit()
...
def get_markets(db_conn_str: str, markets_table: Table) -> pd.DataFrame:
    ...
    markets_df["link1"] = (
        markets_df.loc[markets_df.index.intersection(markets_df["link1"])]
        .reindex(markets_df["link1"])["name"]
        .values
    )
    markets_df["link2"] = (
        markets_df.loc[markets_df.index.intersection(markets_df["link2"])]
        .reindex(markets_df["link2"])["name"]
        .values
    )
    return markets_df


def get_cycle_investments(
    db_conn_str: str,
    uid: str,
    cycle: int,
...
def update_investment_multiplier(
    db_conn_str: str,
    investments_table: Table,
    bid_id: int,
    multiplier: float,
...
def get_stocks(db_conn_str: str, stocks_table: Table) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        dict_results,
        columns=[
            stocks_table.c.price.name,
            stocks_table.c.ticket.name,
            stocks_table.c.cycle.name,
        ],
    )
...
def make_auto_green_investment(
    ...
    engine = create_engine(db_conn_str, future=True)
    last_green_markets = (
        prev_investments_df[prev_investments_df["income"] > 0]
        .groupby("uid")["market"]
        .apply(set)
        .reset_index()
    )
    current_investments = (
        investments_df[investments_df["status"] == 1]
        .groupby("uid")["market"]
        .apply(set)
        .reset_index()
    )
    ...
    for uid in uids:
        lost_green_markets = (
            merged_info.loc[merged_info["uid"] == uid, "market_prev"].item()
            - merged_info.loc[merged_info["uid"] == uid, "market"].item()
        )
...
def finish_cycle(
    ...
    cycle_investments_calculation = calculate_investments(
        investments_df,
        markets_df,
        fund_speed,
    )
    if cycle_investments_calculation is not None:
        ...
        for invest_income in market_incomes:
            update_investment_income(
                db_conn_str,
                investments_table,
                bid_id=invest_income["bid_id"],
                income=invest_income["income"],
            )
            ...
            transaction = Transaction(
                from_=from_,
                to_=to_,
                amount=abs(invest_income["income"]),
                cycle=cycle,
            )
    ...
    for uid, start_bonus in START_BONUSES.items():
        ...
        if start_bonus > 0:
            make_transaction(
                db_conn_str,
                transactions_table,
                Transaction(from_="root", to_=uid, amount=start_bonus, cycle=0),
            )
    stocks_df = pd.DataFrame(
        {"ticket": users_df["ticket"], "price": INIT_STOCK, "cycle": 0},
    )
