...
from qd_cyberpank_game.dashboard.update_state import (
    update_game_state,
...
def dashboard():
    if st.session_state.user is None:
        st.markdown(
            "## Пожалуйста, авторизуйтесь для доступа к операционной информации.",
        )
        with st.sidebar:
            login_form()
    else:
...
if __name__ == "__main__":
    init_state()
    title = (
        f"CP 2.0.20: {st.session_state.user.name}"
        if st.session_state.user
        else "CP 2.0.20: Авторизация"
    )
